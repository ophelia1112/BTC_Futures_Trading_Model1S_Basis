...
# Configure logs to write error
logging.basicConfig(
    filename='**.log',
    level=logging.INFO,
    format='%(asctime)s %(levelname)s %(message)s'
)
# print at terminal at same time
...
DATABASE=os.getenv('**')
if not DB_HOST or not DB_PORT or not DB_USER or not DB_PASSWORD or not DATABASE:
...
def save_dataInto_database(data):
    try:
        ...
        logging.error(f'❌Data insert error:{e}')
...
# Process data structure and transform into df, calculate financial indicators, then save them all in database
async def process_klineData():
    while True:
        ...
        save_dataInto_database(final_data)
