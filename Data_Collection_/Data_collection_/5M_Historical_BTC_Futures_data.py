# historical btc futures 5m data collection
...
def save_data_into_PostgreSQL(data_rows):
    try:
        ...
        logging.error(f'❌PostgreSQL insert data error: {e}')
...
# get data
def get_historical_kline_data(start_date, end_date, symbol='*', interval='*', limit=1000):
    BINANCE_REST_API = '**'
...
def process_originalData_calculate_financial_indicators(klines):
    ...
    save_data_into_PostgreSQL(data_rows)
...
def main():
    ...
    while start_time_ms < end_time_ms:
        batch_end_time = start_time_ms + 1000 *5 * 60_000
        ...
        logging.info(
            f"📦 Current batch time : {transform_timestamp(kline_data[0][0])} → {transform_timestamp(kline_data[-1][0])}")

        process_originalData_calculate_financial_indicators(kline_data)
        last_time = kline_data[-1][0]
        start_time_ms = last_time + 5 * 60_000
        time.sleep(1.2)
